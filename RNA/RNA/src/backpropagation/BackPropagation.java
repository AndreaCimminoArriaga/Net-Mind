...
public class BackPropagation {
	...
	private void processDataInOutputLayer(List<Double> inputData, List<Double> expectedOutput){
		...
		if(results.size() != expectedOutput.size()){
			throw new IllegalArgumentException("Backpropagation (processData) : the expectedOutput ("+expectedOutput.size()+") hasn't the same size as output neurons ("+results.size()+")");
		}
	...
	public NeuralNetwork coachNeuralNetwork() {
		...
		for (List<Double> inputData : this.trainingData.keySet()) {
			...
			// Calculating the squared error for each example in order to
			// calculate the Etr after an epoch.
			...
			TrainingMetrics.squaredErrorsForAnEpochOfTrainingSet.add(error);
