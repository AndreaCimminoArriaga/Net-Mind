...
public class Proben1 {
	...
	private Double processAnEpoch(){
		BackPropagation bPorp =  null;
		try{
			bPorp = new BackPropagation(this.neuralNetwork, data.getTrainingSet());
		}catch(Exception e){
	...
	public NeuralNetwork trainingProcess(){
		System.out.println("");
		System.out.print("- Training the network (epochs course):");
	...
	private Double iterateOverTheValidationSet(){
		...
		//Adding statistics
		if(Statistics.minimumMeanSquaredErrorOnValidation > Eva)
			Statistics.minimumMeanSquaredErrorOnValidation = Eva;
		return Eva;
	}
